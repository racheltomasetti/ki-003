...
## Product Direction (reviewed Aug 2026)

Two LLM-generated roadmaps were reviewed this session — an 11-pillar/4-phase version, then a stripped 5-item version ("Capture → Corpus → Ki Document → Agent Chat → Weekly Reflection, that's it, everything else is what Ki grows into"). Decision: **the stripped version is the right instinct — ship the minimal core loop and let real people in rather than keep expanding scope.** But even the stripped version overstates current reality on two points: "Ki Document... built through onboarding, living and versioned" (onboarding doesn't exist; the document isn't versioned, it's a single mutable field) and "Weekly Reflection" (0% built — no scheduling, no diffing, no review UI). Before inviting real users on this pitch, either ship `/onboarding` first (small, already scoped, directly serves "let people in") or drop those two claims from what's promised until they're true.

**Explicitly deferred, not to be proposed as near-term work** even though they appeared in the reviewed roadmaps: Prompt Library (guided multi-turn creation flows — this is the "recipes" idea from the artifacts conversation, same name different session; gated behind `propose_artifact` actually being reliable, which it wasn't as of this review — see Artifacts-with-tools below), cycle sprint planner, Keepsake/slide-deck artifact kinds, shared/community features. Real ideas, correctly named as far-out — don't scope them until the foundation below is solid.
...
- **Patterns view v1 hardening.** `/patterns` shipped as v0 scaffolding (commit `629516a`) — correct architecture, honest empty states, but it counts words rather than surfacing patterns: no drill-down into evidence, phase- not day-level, agents don't see any of it. Full critique + ordered next steps (pattern objects with `capture_ids`, day-level view, chat-grounding, eventual Postgres aggregation) in `docs/active/patterns-v1.md`. **Not yet started.**

- **`/onboarding` first-run route.** Doesn't exist yet — sign-up currently drops straight into the app with no sequence at all. Spec'd as the natural home for "connect to your cycle" (last period start, avg cycle/period length) and, since it's being built anyway, a first pursuit + core question prompt + first capture rather than an empty Home. See `docs/active/cycle-tracker.md` Phase B. **Elevated priority** — this is the actual front door for "let people in," and closes one of the two reality gaps in the current product pitch (see Product Direction above).
...
- **Artifacts-with-tools reliability.** `propose_artifact` tool calling shipped in both `chat-with-ki` and `pursuit-agent` (Aug 2026) — but live testing confirmed the failure mode the reconciliation pass worried about: Claude would narrate "I'll create that" without actually calling the tool, especially on large/multi-part requests. First fix (prompt language making the tool call mandatory-when-explicitly-asked, not "rare/conservative") took multiple iterations and explicit debugging (`scripts/test-artifact.sh`, diagnostic logging in both functions) to get working reliably. **Treat this as still-fragile, not proven** — the Prompt Library / cycle-native "recipes" layer from `.claude/ARTIFACTS_BUILD_PLAN.md` stays gated behind this being solid across a range of request types, not just the ones tested so far.

- **Weekly Reflection.** Named in both reviewed roadmaps as core to "Ki works end-to-end for one person," but 0% built — no scheduling/trigger mechanism, no Ki-document diffing, no propose/approve/reject review UI. This is the single biggest hidden-scope item in the current product pitch. Not started, not designed.

- **Flow Tracker nesting — reframed from "Ki Cycles."** An old idea from a sibling project (28-day commitment containers, built with Ray's sister, abandoned) resurfaced in the roadmap review as its own pillar. Reframed instead as: this is the parent/child flow nesting Flow Tracker v1 deliberately deferred ("keep it simple... open to exploring layered flows later") — a 28-day container is a long-running parent flow, a daily habit check-in is a child flow inside it. **Preferred starting wedge: anchor on a single morning-routine commitment, not generic "three habits for 28 days"** — narrower, avoids reintroducing multi-habit tracking complexity, and has a real stated rationale (how you start the day sets the tone). Not started.

- **`time_of_day_cat` in the user's local timezone.** Today `enrichments.time_of_day_cat` (`morning` / `afternoon` / `evening` / `night`) is derived in `enrich-capture` from `captured_at` via `Date.getHours()` — never from Claude, which is correct — but the hour is read in the **Edge runtime's timezone (effectively UTC)**, not the user's. Late-evening captures in US zones can stamp as `morning`; early-morning captures in Asia can stamp as `night`. That poisons any later time-of-day pattern work. **Intended behavior:** keep deriving from the capture instant (UTC), never from the model; categorize against the **human's local clock** (same spirit as cycle day — stamp the truth, interpret in lived context). Buckets can stay as they are. **Not yet started.**
