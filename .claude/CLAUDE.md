...
- **Artifacts-with-tools.** A full plan already exists at `.claude/ARTIFACTS_BUILD_PLAN.md` (locked May 2026: `propose_artifact` tool calling, discriminated `Artifact` union, prose/outline/mermaid kinds, PR-sequenced) — but it predates the pursuit-agent's current `<response>`/`<citations>` contract and **needs a reconciliation pass against current code before execution**, not a fresh design. Backend is partially there (`pursuit_artifacts.data` column, typed `createPursuitArtifact`); frontend (`ArtifactSurface`, `ThoughtDistiller` replacement) and the agent's tool-calling contract are fully unbuilt. Separately, cycle-native "recipes" (script/plan/schedule kinds grounded in cycle patterns, e.g. a release meditation from a recurring luteal doubt cluster) are a layer intended to sit on top of this once it's proven — not before. **Sequencing against the Patterns v1 hardening above is undecided — resolve before starting either.**
