...
1. **"One prompt" on the home page vs. the locked chat rule.** `project_explore_surface.md` locks chat out of top-level nav — it only lives in the pursuit workspace sidebar. A single Ki-generated prompt on Home isn't full chat, but it's adjacent enough to the line that it's worth deciding deliberately: is this a question with no reply box (pure reflection), or does answering it open a conversation? If the latter, where does that conversation go?
